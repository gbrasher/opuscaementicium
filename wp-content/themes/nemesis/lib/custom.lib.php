...
{ 
    ...
    { 
        ...
        $next = ($currentPage + 1 > $totalPages) ? false : '<a href="'.$baseLink.$start.'page='.($currentPage + 1).'" class="slide">'._e( 'Next', THEMEDOMAIN ).'</a>'; 
         
        $prev = ($currentPage - 1 <= 0 ) ? false : '<a href="'.$baseLink.$start.'page='.($currentPage - 1).'" class="slide">'._e( 'Previous', THEMEDOMAIN ).'</a>'; 
    } 
	...
	if(!empty($posts))
	{
		...
		$return_html.= '<ul class="posts blog '.$bg_color.'_wrapper">';
			...
			foreach($posts as $post)
			{
				...
				{
					$return_html.= '<a href="'.get_permalink($post->ID).'"><img src="'.get_bloginfo( 'stylesheet_directory' ).'/timthumb.php?src='.$image_thumb[0].'&amp;h=60&amp;w=60&amp;zc=1" alt="" class="img_nofade frame"/></a>';
				}
				$return_html.= '<a href="'.get_permalink($post->ID).'">'.$post->post_title.'</a><br/>';
				$return_html.= date('F j, Y', strtotime($post->post_date)).'</li>';
