function removeSortRecord(thisParentLi, targetObj)
{
    ...
    jQuery(document).ready(function(){
		...
		jQuery('#save_ppsettings').click(function(){
			jQuery('#pp_loading').fadeIn();
			sTarget = jQuery(this).attr('href');
			dataString = jQuery("#pp_form").serialize();
			...
			jQuery.ajax({
	            ...
	            data: dataString,  
	            success: function() {  
	            	jQuery('#pp_loading').hide();
	            	jQuery('#pp_success').fadeIn();
		...
		jQuery("#portfolio_type").change(function(){
			...
			else if(currentVal=="Portfolio Content")
			{
				jQuery('#portfolio_gallery_id_wrapper').fadeIn();
			}
			else if(currentVal=="AJAX Portfolio Content")
			{
				jQuery('#portfolio_gallery_id_wrapper').fadeIn();
			}
		});
